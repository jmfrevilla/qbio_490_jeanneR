...
```{r}
write.csv(clinic, "/Users/mi/Desktop/QBIO/qbio_490_jeanneR/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/Users/mi/Desktop/QBIO/qbio_490_jeanneR/analysis_data/brca_clinical_data.csv")
```
...
```{r Exercise 4.2}
# create plot here
boxplot(clinic$age_at_diagnosis)
...
```{r}
cleaned_clinic$age_category <- cleaned_clinic$age_category <- ifelse(clinic$age_at_diagnosis >=50, "Old", ifelse(clinic$age_at_diagnosis > 35, "Middle", "Young"))
```
...
```{r}
cleaned_clinic$survival_time <- ifelse(cleaned_clinic$vital_status == 'Alive', cleaned_clinic$last_contact_days_to, cleaned_clinic$death_days_to)
```
...
p = 0.2
This is not statistically significant. It tells us there is a 20% chance of this data occurring if the null hypothesis (age does not affect survival time) is true. There is low correlation between age group and survival time.
